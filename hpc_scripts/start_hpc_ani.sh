...
## 15-02-2014 pieter.stragier@ugent.be v0.1
...
#@todo make this in arguments/options when done
#############################################################################################
###  Variables   ###
#############################################################################################
...
##$VSC_SCRATCH_NODE: local (on blade) scratch directory 85GB shared ( For all tmp and intermidair steps when staien on the node)
...
project_name=ANI
ani_script=$VSC_HOME/ANI/ani.py
input_folder=$VSC_DATA/ANI/fastafiles
output_folder=$VSC_DATA/ANI/jobs
tmp_dir=$VSC_SCRATCH/ANI/$project_name/tmp
selection=["LMG23076", "LMG982"]
...
python $ani_script -v -i $input_folder -o $output_folder -s $selection
